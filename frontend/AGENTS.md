...
- AgentQA execution/comparison attachments are extracted in `src/core/messages/utils.ts` from the completed ToolMessage matching the tool-call ID. `message-list.tsx` supplies the group's messages. Never synthesize latest-report paths for pending/failed calls; historical runs must retain their archived paths. Tests live in `tests/unit/core/messages/agentqa-artifacts.test.ts` and `tests/e2e/agentqa-regression-artifacts.spec.ts`.
