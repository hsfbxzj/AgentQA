...
- **AgentQA Demo API** (host port 8003, container port 8000): Local-only FastAPI contract-testing target with five business operations and three intentional defects
...
│   ├── agentqa/               # AgentQA OpenAPI parser, test engine, reports, and LangChain tools
│   ├── agentqa_demo/          # Local AgentQA API test target
│   ├── tests/                 # Test suite
│   └── docs/                  # Documentation
...
`backend/agentqa_demo/` is a deterministic local test target for the AgentQA learning extension. It exposes five OpenAPI business operations while `/health` is intentionally omitted from the schema. Three contract defects are deliberately preserved and identified as `DEMO-BUG-001` through `DEMO-BUG-003`.

- Host Swagger UI: `http://localhost:8003/docs`
- Host OpenAPI: `http://localhost:8003/openapi.json`
- Docker-internal URL: `http://agentqa-demo-api:8000`
- Focused tests: `PYTHONPATH=. uv run pytest tests/agentqa_demo -q`
- Expected result: `6 passed, 3 xfailed`

The strict `xfail` contract tests are part of the fixture design. Do not fix a seeded defect without updating the AgentQA teaching scenario, its defect catalog, and the associated tests. `scripts/docker.sh start` includes the `agentqa-demo-api` service in every Docker development sandbox mode.

### AgentQA Testing Agent

`backend/agentqa/` is the domain-specific testing core added on top of DeerFlow. It keeps pass/fail decisions deterministic while DeerFlow's lead agent handles natural-language interaction and orchestration.

- `models.py`: strict Pydantic contracts for normalized OpenAPI operations, plans, cases, results, versioned execution reports, and regression comparisons
- `openapi_parser.py`: OpenAPI 3.x JSON/YAML parser with local component-reference resolution
- `case_generator.py`: deterministic one-case-per-operation rule engine and stable SHA-256-derived plan IDs
- `security.py`: local target allowlist and request-path validation
- `executor.py` + `assertions.py`: approval-gated HTTP execution and programmatic contract assertions
- `regression.py`: pure recorded-result comparison; validates counts, snapshots, and fingerprints, matches comparable cases, and separates coverage/condition changes and network errors
- `reporter.py`: execution and regression JSON/Markdown output with before/after evidence
- `tools.py`: `agentqa_create_test_plan`, `agentqa_execute_test_plan`, and `agentqa_compare_test_reports`; report files are written off the async event loop and returned in a `Command` with actual artifact paths
- `../skills/public/agentqa-api-testing/SKILL.md`: mandatory plan → clarification → approval → execution → report workflow

The tools are registered in `config.yaml`/`config.example.yaml` under the `agentqa` group. Docker `NO_PROXY` must include `agentqa-demo-api` so the Gateway can reach the Compose service directly. The MVP allowlist intentionally accepts only `localhost`, `127.0.0.1`, and `agentqa-demo-api`.
...
The AgentQA integration expectation is five generated cases with two passes and three failures, one failed case for each seeded contract defect. Execution without `approved=True` must raise `ApprovalRequiredError` before any target request is sent. Each approved execution creates a UUID run ID, a version-2 report with request/assertion fingerprints and a plan snapshot, and immutable `agentqa/runs/<run_id>/{agentqa-report.md,agentqa-report.json,agentqa-plan.json}` artifacts. Root report names are latest aliases only. Artifact delivery must happen in the same tool `Command` and must not depend on a later LLM call.

Regression comparisons take two report JSON strings (raw reports or execution-tool wrappers), issue no HTTP requests, and archive `agentqa/comparisons/<comparison_id>/{agentqa-regression.md,agentqa-regression.json}`. Match version-2 results by recomputed fingerprint, excluding display ID/name; reject duplicate IDs/fingerprints and inconsistent snapshots/counts. Old reports require matching plan IDs and unambiguous case metadata, with an explicit limitation warning. Targets and API titles must agree. Count failed cases, not inferred root causes; missing tests and changed conditions must never count as fixed. Network failures are execution errors. Frontend attachments read the matching completed ToolMessage's `artifacts` so history never points to the latest alias. The simulated repair scenario in `tests/agentqa/test_regression.py` must produce fixed=1, new_defects=1, unresolved=2, persistent_passes=1 without changing the seeded Demo fixtures.
